...
def get_image_filename_list(root) -> list:
    ...
    if os.path.exists(img_dir) is not True or os.path.exists(
            dot_dir) is not True:
    ...
    try:
        ...
        raise FileNotFoundError(
            "Cannot find json file of filenames in {}".format(root))
...
class Cell_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(img).float().unsqueeze(0), torch.from_numpy(
            dot).float().unsqueeze(0), count
